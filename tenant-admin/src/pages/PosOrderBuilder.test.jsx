...
describe("PosOrderBuilder - seeding a cart from a reorder", () => {

    const REORDER_CUSTOMER = { CustomerId: 12, FullName: "Vaibhav Nawale", Phone: "9876543210" };

    it("attaches the named customer and adds only items still on the menu, at today's price", async () => {

        renderBuilder({
            initialReorder: {
                sourceOrderId: 94,
                customer: REORDER_CUSTOMER,
                items: [
                    { menuItemId: CHAI.MenuItemId, quantity: 2, hadOptions: false },
                    // 999 no longer exists on this branch's active menu at all.
                    { menuItemId: 999, quantity: 1, hadOptions: false }
                ]
            }
        });
        ...
        // The named customer from the reorder, not the usual Guest default.
        await screen.findByText(/Vaibhav Nawale.*9876543210/);

        // 2x Ginger Chai at its current ₹30 price - the missing item is
        // silently skipped rather than blocking the rest.
        expect(await screen.findByText(/subtotal: ₹\s*60\.00/i)).toBeInTheDocument();

    });

    it("skips a customized line rather than guessing its options back together", async () => {
        ...
        renderBuilder({
            initialReorder: {
                ...
                customer: REORDER_CUSTOMER,
                items: [
                    { menuItemId: CHAI.MenuItemId, quantity: 1, hadOptions: false },
                    { menuItemId: PIZZA.MenuItemId, quantity: 1, hadOptions: true }
                ]
            }
        });
        ...
        // Only the plain Chai line was seeded - Pizza had customization on
        // the original order, so it's left for staff to re-add deliberately.
        expect(await screen.findByText(/subtotal: ₹\s*30\.00/i)).toBeInTheDocument();

    });

    // reorderSeededRef guards this - without it, any unrelated re-render
    // (typing in the item search box, in this case) would re-run the
    // seeding effect and silently resurrect a line staff had just removed.
    it("does not re-seed the cart on an unrelated re-render after staff has already edited it", async () => {
        ...
        renderBuilder({
            initialReorder: {
                sourceOrderId: 96,
                customer: REORDER_CUSTOMER,
                items: [{ menuItemId: CHAI.MenuItemId, quantity: 1, hadOptions: false }]
            }
        });

        await screen.findByText("Ginger Chai");
        await waitFor(() => expect(screen.getByText(/subtotal: ₹\s*30\.00/i)).toBeInTheDocument());

        // Staff decides against the seeded Chai and removes its cart line
        // entirely (the cart panel's own trash icon, not the menu tile's
        // stepper - only one cart line exists here, so this is unambiguous).
        // Subtotal only renders while the cart is non-empty, so an empty
        // cart is "Subtotal" being absent altogether, not "₹0.00".
        await user.click(screen.getByTestId("DeleteRoundedIcon"));
        await waitFor(() => expect(screen.queryByText(/subtotal:/i)).not.toBeInTheDocument());

        // An unrelated re-render (typing in search) must not bring it back.
        await user.type(screen.getByPlaceholderText(/search/i), "chai");
